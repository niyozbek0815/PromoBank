                    ...
                    <form action="{{ route('admin.promotion_shops.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            @error('name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                            <small class="text-muted">Do‘kon nomi xarid chekida qanday bo‘lsa, aynan shunday
                                kiriting.</small>
                        </div>

                        {{-- 📍 Manzil --}}
                        <div class="mb-3">
                            <label class="form-label">Manzil <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="adress" rows="3"
                                placeholder="Masalan: Toshkent sh., Chilonzor tumani, Qatortol ko‘chasi, 12-uy" required>{{ old('adress') }}</textarea>
                            @error('adress')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="
         @if (!empty($selected_promotion)) {{ route('admin.promotion.edit', $selected_promotion) }}
            @else
