...
@section('content')
            ...
            <div class="container-sm">
                <div class="promotion-header">
                    <div class="name-company">
                        <p class="sub-title">{{ $promotion['title'] ?? '' }}</p>
                        <h2 class="section-title">{{ $promotion['name'] }}</h2>
                        <div class="promotion-dates">
                            <div class="start">
                                <span class="date-label">Boshlanish:</span>
                                <span
                                    class="date-value">{{ \Carbon\Carbon::parse($promotion['start_date'])->format('d.m.Y') }}</span>
                            </div>
                            <div>
                                <span class="date-label">Tugash:</span>
                                <span
                                    class="date-value">{{ \Carbon\Carbon::parse($promotion['end_date'])->format('d.m.Y') }}</span>
                            </div>
                            @if (!empty($promotion['offer']))
                                <a href="{{ $promotion['offer'] }}" target="_blank" class="offer-link">
                                    <i class="fa-regular fa-file-lines"></i> Oferta bilan tanishing
                                </a>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="company-header">
                            ...
                            <div class="company-details">
                                <h5 class="company-name">
                                    {{ $promotion['company']['name'][$locale] ?? $promotion['company']['name']['uz'] }}</h5>
                    ...
                    <div class="participation-methods">
                        <h5 class="methods-title">Qatnashish usullari</h5>
                        <div class="methods-grid">
                            ...
                            @if (in_array('sms', $platforms))
                                <a href="#" class="method-btn" onclick="openSmsModal(event)">
                                    ...
                                    <span>SMS orqali qatnashish</span>
                                </a>
                            @endif
                ...
                @if (!empty($promotion['gallery']))
                    <div class="media-section">
                        <h3 class="media-title">Promoaksiya Media Galereyasi</h3>
    ...
    @if ($smsPlatform)
        <div id="smsModal" class="scannerModal">
            <div class="modal-content">
                <h4>SMS orqali qatnashish</h4>
                <p class="mt-3 mb-3">
                    Promokodeni <strong>{{ $smsPlatform['phone'] ?? 'XXXX' }}</strong> qisqa raqamiga yuboring.
                </p>
    ...
    <div id="scannerModal" class="scannerModal">
        <div class="modal-content">
            <h4>📱 {{ $download['title'] ?? 'Mobil ilovani yuklab oling' }}</h4>

            <p class="mt-2">
                {{ $download['description'] ?? 'PromoBank mobil ilovasi va Telegram bot orqali barcha aksiyalarda qatnashing.' }}
            </p>
            ...
            <button type="button" class="btn-close-modal mt-3" onclick="closeScannerModal()">
                Yopish
            </button>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <button type="button" class="btn-close-modal mt-3" onclick="closeReceiptModal()">
                Yopish
            </button>
        </div>
    </div>
    ...
    <div id="codeModal" class="scannerModal" style="display:none;">
        ...
        <div class="modal-content">
            ...
            <button type="button" class="btn-close-modal mt-3" onclick="closeCodeModal()">
                Yopish
            </button>
        </div>
    </div>
